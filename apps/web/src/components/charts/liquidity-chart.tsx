  ...
  if (data.length < 2) {
    return (
      <div className="flex h-56 items-center justify-center text-sm text-muted-foreground">
        Not enough indexed history yet — check back after a few trades.
      </div>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        ...
        />
        <Area type="monotone" dataKey="liquidityUsd" stroke="#22c55e" fill="url(#liquidityFill)" strokeWidth={2} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
