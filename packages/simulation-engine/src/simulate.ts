...
import { simulateAtCurvePosition, getCurvePositionStartPrice, getQuoteReserveAtSqrtPrice } from "@elf/meteora-adapter";
import type {
  CurveCandidate,
  MarketProfile,
  SimulationRun,
...
import { SIMULATION_MAX_IMPACT_BPS, TRADE_SIZES_USD } from "@elf/shared";
...
/**
 * Runs ONE scenario definition (any of the engine's own, or one whose position
 * / trade sides the Simulation Lab has overridden and validated) across the
 * four standard trade sizes. Pure curve math: no chain access, no signing.
 */
export function runScenario(params: RunSimulationParams, scenario: ScenarioDefinition): SimulationScenarioResult {
  ...
  const { startPriceInQuote, startSqrtPrice } = getCurvePositionStartPrice(
    configParameters,
  ...
  const toUsd = (raw: BN) => fromLamports(raw, tokenQuoteDecimal) * quoteUsdPrice;
  const startQuoteReserveUsd = toUsd(getQuoteReserveAtSqrtPrice(configParameters, startSqrtPrice));
  ...
  const trades: SimulationTradeResult[] = TRADE_SIZES_USD.map((tradeSizeUsd, index) => {
    ...
    let reserveQuoteAfterUsd: number | null = null;
    ...
    try {
      ...
      reserveQuoteAfterUsd = toUsd(getQuoteReserveAtSqrtPrice(configParameters, simulated.quote.nextSqrtPrice));
    } catch (error) {
    ...
    const result: SimulationTradeResult = {
      ...
      reserveQuoteAfter: reserveQuoteAfterUsd === null ? null : round2(reserveQuoteAfterUsd),
  ...
  return {
    ...
    curveProgressFraction: scenario.curveProgressFraction,
    startPriceUsd: roundPrice(startPriceUsd),
    startQuoteReserveUsd: round2(startQuoteReserveUsd),
    migrationThresholdUsd: round2(toUsd(configParameters.migrationQuoteThreshold)),
  };
}
...
export function runSimulation(params: RunSimulationParams): SimulationRun {
  ...
  const scenarios = SCENARIO_DEFINITIONS.map((scenario) => runScenario(params, scenario));
